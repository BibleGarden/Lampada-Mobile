...
test('нумерация дней: ISO понедельник становится expo-вторником, воскресенье — единицей', () => {
  // expo-notifications: 1 = воскресенье … 7 = суббота.
  assert.deepEqual([1, 2, 3, 4, 5, 6, 7].map(expoWeekday), [2, 3, 4, 5, 6, 7, 1]);
});

test('нормализация чистит мусор, сортирует и убирает дубли', () => {
...
test('нормализация сливает правила с одинаковым набором дней и роняет пустые', () => {
  const schedule = normalizeReminderSchedule({
    enabled: true,
    rules: [
...
test('нормализация обрезает число времён в одном правиле', () => {
...
test('нормализация отвергает значение, не похожее на расписание', () => {
...
test('повреждённое сохранённое значение не считается расписанием', () => {
...
test('разворачивает правила в WEEKLY-триггеры в нумерации expo', () => {
...
test('пересекающиеся правила не дают двух уведомлений в один момент', () => {
  const triggers = reminderWeeklyTriggers(enabled([
...
test('выключенное расписание не даёт ни одного триггера', () => {
...
test('максимум, достижимый из интерфейса, укладывается в лимит iOS', () => {
...
test('расписание сложнее лимита iOS обрезается, а не теряется в системе', () => {
...
test('рендерит расписание человеческой строкой', () => {
  const schedule = normalizeReminderSchedule({
    enabled: true,
    rules: [
...
test('группирует дни в диапазоны и называет полную неделю', () => {
...
test('время рендерится с ведущим нулём, пустое расписание — пустой строкой', () => {
  assert.equal(
...
test('перетасовка сохраняет весь пул и переставляет его', () => {
...
test('перетасовка не выходит за границы пула на краевых значениях случайности', () => {
