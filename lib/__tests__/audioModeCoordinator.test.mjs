...
test('recording lease synchronously skips global audio-session deactivation', async () => {
  const coordinator = createAudioModeCoordinator();
  const lease = coordinator.acquireRecording(async () => undefined, recordingMode);
  let deactivations = 0;

  const deactivated = await coordinator.requestDeactivation(async () => {
    deactivations += 1;
  });
...
test('recording supersedes an older queued session deactivation', async () => {
  const coordinator = createAudioModeCoordinator();
  let releasePlayback;
  const playbackGate = new Promise((resolve) => {
    releasePlayback = resolve;
  });
  const playback = coordinator.requestPlayback(async () => playbackGate, playbackMode);
  await Promise.resolve();
  let deactivations = 0;
  const deactivation = coordinator.requestDeactivation(async () => {
    deactivations += 1;
  });
  const lease = coordinator.acquireRecording(async () => undefined, recordingMode);
  ...
  releasePlayback();
  await playback;
  assert.equal(await deactivation, false);
  await lease.ready;
  assert.equal(deactivations, 0);
});
