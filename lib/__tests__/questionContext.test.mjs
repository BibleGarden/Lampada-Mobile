...
const settingsUrl = `data:text/javascript,${encodeURIComponent(`
  ...
  let answers = true;
  let uiLanguage = 'en';
  export const useSettings = { getState: () => ({ uiLanguage, scripturePreferences: { language: 'uk' } }) };
...
const hooks = registerHooks({
  resolve(specifier, context, nextResolve) {
    ...
    if (context.parentURL?.endsWith('/store.ts')) {
      const mocks = {
        './db': 'export const saveAnswer = async () => {}; export const replaceRecordings = async () => {};',
        './scriptureRepository': '',
        './scriptureClient': 'export const fetchScriptureBooks = () => {}; export const selectScripture = () => {}; export const selectScriptureOnce = () => {};',
...
test('session first, replace, next and reflect requests use UI language and discard prefetches from another language', async () => {
  ...
  try {
    ...
    await settle();
    assert.deepEqual(requests, [{ stage: 'first', topic: '', messages: [], default_language: 'ru' }]);
    ...
    const reply = 'Я вдячна за підтримку моєї родини.';
    useSession.getState().saveAnswer(0, reply, []);
    await useSession.getState().nextQuestion();
    await settle();
