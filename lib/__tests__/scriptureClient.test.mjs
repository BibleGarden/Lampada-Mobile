...
test('book catalog URL preserves the injected test server', () => {
  assert.equal(
    resolveScriptureBooksUrl('http://192.168.1.2:9084/api/ai/scripture', 1),
...
test('loads and validates translation book names for exact passage references', async () => {
  ...
  const books = await fetchScriptureBooks(1, {
    url: 'https://proxy.test/api/ai/scripture',
    apiKey: 'public-key',
...
test('single request sends the configured public key and accepts an unknown source', async () => {
  ...
  assert.equal(calls[0].url, 'https://proxy.test/api/ai/scripture');
...
for (const detail of ['prefetch_disabled', 'prefetch_limit_exceeded']) {
  test(`prefetch ${detail} is silent, marked and never retried`, async () => {
    const requests = [];
    const sleeps = [];
    const result = await selectScripture({ language: 'ru', translation: 1, prefetch: true }, {
      ...noTimeout,
      url: 'https://proxy.test/api/ai/scripture',
      fetchImpl: async (_url, init) => {
        requests.push(JSON.parse(init.body));
        return jsonResponse({ detail }, { status: 429, headers: { 'retry-after': '30' } });
      },
      sleep: async (milliseconds) => sleeps.push(milliseconds),
    });
    assert.deepEqual(result, { ok: false, error: { kind: 'prefetch_denied' } });
    assert.equal(requests.length, 1);
    assert.equal(requests[0].prefetch, true);
    assert.deepEqual(sleeps, []);
  });
}

test('prefetch never retries ordinary quota or transport failures', async () => {
  for (const status of [429, 503]) {
    let calls = 0;
    const result = await selectScripture({ language: 'ru', translation: 1, prefetch: true }, {
      ...noTimeout,
      url: 'https://proxy.test/api/ai/scripture',
      fetchImpl: async () => { calls++; return jsonResponse({ detail: 'Unavailable' }, { status }); },
      sleep: async () => assert.fail('Background requests must not retry'),
    });
    assert.equal(result.ok, false);
    assert.equal(calls, 1);
  }
