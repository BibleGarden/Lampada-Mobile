...
test('uses an explicit URL or derives scripture endpoint from the question endpoint', () => {
  assert.equal(
    resolveScriptureUrl(
      ' https://scripture.test/custom ',
      'https://proxy.test/api/ai/question',
    ),
    'https://scripture.test/custom',
  );
  assert.equal(
    resolveScriptureUrl(undefined, 'http://192.168.1.2:9084/api/ai/question'),
    'http://192.168.1.2:9084/api/ai/scripture',
  );
  assert.equal(resolveScriptureUrl(undefined, 'https://proxy.test/api/ai/not-question'), null);
  assert.equal(
