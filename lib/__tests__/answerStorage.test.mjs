...
const sqliteUrl = data(`
  export const state = { rows: [], sharedRows: [], openExec: [], txnLog: [], failRecordingInsert: false };
  export const openDatabaseAsync = async () => ({
    execAsync: async (sql) => { state.openExec.push(sql); },
    ...
    withExclusiveTransactionAsync: async (task) => {
      const pending = [];
      const txn = {
        execAsync: async (sql) => { state.txnLog.push(sql); },
        getAllAsync: async (sql) => { state.txnLog.push(sql); return []; },
        runAsync: async (sql, ...args) => {
          state.txnLog.push(sql);
...
test('both connections wait for the write lock and the transaction writes before it reads', async () => {
  await db.getDb();
  assert.ok(state.openExec.some((sql) => sql.includes('PRAGMA busy_timeout = 5000;')));
  state.txnLog.length = 0;
  await db.saveAnswer(answer, [recording]);
  assert.equal(state.txnLog[0], 'PRAGMA busy_timeout = 5000;');
  assert.match(state.txnLog[1], /^INSERT INTO answers/);
});
