...
По умолчанию текст ответов отправляется при генерации следующих вопросов. Пользователь может отключить передачу настройкой «Учитывать мои ответы», сохранённой в `meta.share_answers`: отсутствие значения либо `1` означает передачу, `0` — запрет. Тема молитвы и уже заданные вопросы могут входить в запрос независимо от этой настройки.
